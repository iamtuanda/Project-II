        ...
        <div class="row product-element">
            <div class="col-12 col-sm-12 col-md-12 col-lg-">
                <div id="search-product-title">
                    <div class="search-product-key" data-id="<?php echo $categories_types_id; ?>">View ' <span><?php echo $view; ?></span> ': </div>
                    <div class="search-product-result"><span><?php echo $count; ?></span> results</div>
                </div>
            </div>
        </div>
            ...
            <?php foreach ($products as $key => $product): ?>
                ...
                <a class="col-sm-6 col-md-4 col-lg-3 product-result" href="product_detail?p=<?php echo base64_encode($product['tProduct']['id']); ?>">
                    ...
                    <?php if ($product['tProduct']['price'] != $product['tProduct']['finish_price']) { ?>
                        <p class="product-price-before"><strike><span><?php echo $product['tProduct']['price']; ?></span> VNĐ</strike></p>
                    <?php } else { ?>
                        <p class="empty"></p>
                    <?php } ?>
                    <p class="product-last-price"><span><?php echo $product['tProduct']['finish_price']; ?></span></p>
                    <div class="btn btn-outline-danger product-add-to-cart"> <i class="fas fa-cart-plus"></i> Add to cart</div>
                </a>
            <?php endforeach; ?>
        </div>
        <?php if (sizeof($products) != 0 && $page > 1): ?>
